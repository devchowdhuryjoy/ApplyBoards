...
// const AgentFinalApply = ({ id, onClose }: { id: string; onClose?: () => void }) => {
...
//         const response = await fetch(`${BASE_URL}/agent/applications/${id}/detail`, requestOptions);
...
//           const parsedData = {
//             ...application,
//             academic_qualifications: parseJsonField(application.academic_qualifications),
...
//           if (application.status === "Submitted" || application.status === "Approved" || application.status === "Processed" || application.status === "Reviewed") {
...
//       newErrors[step] = newErrors[step].filter(error => !error.includes(name));
//       setStepErrors(newErrors);
//     }
//   };

//   // Handle array field changes
//   const handleArrayChange = (field: keyof FormData, index: number, subField: string, value: string) => {
//     setFormData(prev => {
...
//   // Handle file changes
//   const handleFileChange = (e: ChangeEvent<HTMLInputElement>, field: keyof FileState) => {
...
//         if (!formData.student_id.toString().trim()) errors.push('Student ID is required');
//         if (!formData.dob) errors.push('Date of Birth is required');
//         if (!formData.gender) errors.push('Gender is required');
//         if (!formData.passport.trim()) errors.push('Passport Number is required');
//         if (!formData.passport_expiry) errors.push('Passport Expiry Date is required');
//         break;
...
//       case 3:
//         if (!formData.last_level_of_study.trim()) errors.push('Last Level of Study is required');
//         if (!formData.grading_scheme.trim()) errors.push('Grading Scheme is required');
//         if (!formData.study_level.trim()) errors.push('Study Level is required');
//         break;
        
//       case 6:
//         // Document validation
//         const requiredDocs = ['resume', 'passport_copy', 'transcripts'] as const;
...
//       case 9:
//         if (formData.academic_qualifications.length === 5) {
//           errors.push('At least one Academic Qualification is required');
//         }
//         if (!formData.sop.trim()) errors.push('Statement of Purpose (SOP) is required');
//         break;
//     }
...
//       1: ['student_name', 'student_id', 'dob', 'gender', 'passport', 'passport_expiry'],
//       2: ['email', 'phone', 'address', 'country_of_residence', 'student_profile_nationality'],
//       3: ['last_level_of_study', 'grading_scheme', 'study_level', 'subject', 'specialization', 'education_country', 'field_of_study_name', 'program_level'],
//       4: ['program_id', 'program_name', 'university_name', 'intake', 'status', 'agent_name'],
//     };
...
//       showErrorAlert('This application has already been submitted. Please contact your admin for any changes.');
//       return;
//     }
...
//       // Add all form fields
//       const formFields = [
//         'student_name', 'student_id', 'email', 'phone', 'address', 'gender', 'dob',
//         'passport', 'passport_expiry', 'country_of_residence', 'student_profile_nationality',
//         'last_level_of_study', 'grading_scheme', 'study_level', 'subject', 'specialization',
//         'education_country', 'university_name', 'program_name', 'intake', 'status',
//         'field_of_study_name', 'program_level', 'ielts_required', 'ielts_reading',
//         'ielts_writing', 'ielts_listening', 'ielts_speaking', 'ielts_overall',
//         'toefl_required', 'toefl_reading', 'toefl_writing', 'toefl_listening',
//         'toefl_speaking', 'toefl_overall', 'has_gre_score', 'has_gmat_score',
//         'company_name', 'achievements', 'sop'
//       ];
...
//       formDataToSend.append('academic_qualifications', JSON.stringify(formData.academic_qualifications));
//       formDataToSend.append('test_scores', JSON.stringify(formData.test_scores));
//       formDataToSend.append('work_experiences', JSON.stringify(formData.work_experiences));
...
//       const response = await fetch(`${BASE_URL}/agent/applications/${id}`, requestOptions);
//       const result = await response.json();
...
//   const isFormDisabled = submitSuccess || alreadySubmitted;
...
//               Status: <span className={`font-semibold ${formData.status === 'Submitted' ? 'text-green-600' : 'text-blue-600'}`}>
//                 {formData.status}
//               </span>
//             </p>
...
//                 <div className={`w-6 h-6 rounded-full flex items-center justify-center mb-1 text-xs ${
//                   currentStep > index + 1 
...
//                   currentStep >= index + 1 ? 'font-medium text-gray-800' : 'text-gray-500'
//                 }`}>
//                   {step.split(' ')[0]}
//                 </span>
//               </div>
...
//             </span>
//             {stepErrors[currentStep] && stepErrors[currentStep].length > 0 && !isFormDisabled && (
...
//                     {submitSuccess ? 'Application submitted successfully!' : 'This application has already been submitted.'}
//                   </p>
//                   <p className="text-sm text-blue-600 mt-1">
//                     Please contact your admin for any changes. You cannot modify a submitted application.
//                   </p>
//                 </div>
//               </div>
//             </div>
...
//                       1
//                     </div>
//                     <h3 className="text-lg font-semibold text-secondary">Student Personal Information</h3>
//                   </div>
                  
//                   <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
//                     {[
//                       { label: "Student Name *", name: "student_name", type: "text", value: formData.student_name },
//                       { label: "Student ID *", name: "student_id", type: "text", value: formData.student_id },
//                       { label: "Date of Birth *", name: "dob", type: "date", value: formData.dob },
//                       { label: "Gender *", name: "gender", type: "select", value: formData.gender, options: ["Male", "Female", "Other"] },
//                       { label: "Passport Number *", name: "passport", type: "text", value: formData.passport },
//                       { label: "Passport Expiry Date *", name: "passport_expiry", type: "date", value: formData.passport_expiry },
//                     ].map((field) => (
//                       <div key={field.name}>
...
//                       2
//                     </div>
//                     <h3 className="text-lg font-semibold text-secondary">Contact Details</h3>
//                   </div>
...
//                       { label: "Country of Residence", name: "country_of_residence", type: "text", value: formData.country_of_residence },
//                       { label: "Nationality", name: "student_profile_nationality", type: "text", value: formData.student_profile_nationality },
//                     ].map((field) => (
//                       <div key={field.name}>
//                         <label className="block text-sm font-medium text-gray-700 mb-1">
//                           {field.label}
...
//                       3
//                     </div>
//                     <h3 className="text-lg font-semibold text-secondary">Academic Information</h3>
//                   </div>
                  
//                   <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
//                     {[
//                       { label: "Last Level of Study *", name: "last_level_of_study", type: "text", value: formData.last_level_of_study },
//                       { label: "Grading Scheme *", name: "grading_scheme", type: "text", value: formData.grading_scheme },
//                       { label: "Study Level *", name: "study_level", type: "text", value: formData.study_level },
//                       { label: "Subject", name: "subject", type: "text", value: formData.subject },
//                       { label: "Specialization", name: "specialization", type: "text", value: formData.specialization },
//                       { label: "Education Country", name: "education_country", type: "text", value: formData.education_country },
//                       { label: "Field of Study Name", name: "field_of_study_name", type: "text", value: formData.field_of_study_name },
//                       { label: "Program Level", name: "program_level", type: "text", value: formData.program_level },
//                     ].map((field) => (
//                       <div key={field.name}>
//                         <label className="block text-sm font-medium text-gray-700 mb-1">
//                           {field.label}
...
//                       4
//                     </div>
//                     <h3 className="text-lg font-semibold text-secondary">Program Details</h3>
//                   </div>
                  
//                   <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
//                     {[
//                       { label: "Program ID", name: "program_id", type: "text", value: formData.program_id },
//                       { label: "Program Name", name: "program_name", type: "text", value: formData.program_name },
//                       { label: "University Name", name: "university_name", type: "text", value: formData.university_name },
//                       { label: "Intake", name: "intake", type: "text", value: formData.intake },
//                       { label: "Status", name: "status", type: "text", value: formData.status, readOnly: true },
//                       { label: "Agent Name", name: "agent_name", type: "text", value: formData.agent_name, readOnly: true },
//                     ].map((field) => (
//                       <div key={field.name}>
//                         <label className="block text-sm font-medium text-gray-700 mb-1">
//                           {field.label}
//                         </label>
//                         {isFormDisabled || field.readOnly ? (
//                           <div className="w-full px-3 py-2 border border-gray-300 rounded-lg text-sm bg-gray-100 text-gray-700">
//                             {field.value || 'Not provided'}
//                           </div>
//                         ) : (
...
//                       5
//                     </div>
//                     <h3 className="text-lg font-semibold text-secondary">Language Requirements</h3>
//                   </div>
...
//                           />
//                         )}
//                         <label className={`ml-2 text-sm font-medium ${isFormDisabled ? 'text-gray-700' : 'text-gray-700'}`}>
//                           IELTS Required: {formData.ielts_required ? 'Yes' : 'No'}
//                         </label>
//                       </div>
//                     </div>
...
//                         {[
//                           { label: "Reading", name: "ielts_reading", value: formData.ielts_reading },
//                           { label: "Writing", name: "ielts_writing", value: formData.ielts_writing },
//                           { label: "Listening", name: "ielts_listening", value: formData.ielts_listening },
//                           { label: "Speaking", name: "ielts_speaking", value: formData.ielts_speaking },
//                           { label: "Overall", name: "ielts_overall", value: formData.ielts_overall },
//                         ].map((field) => (
//                           <div key={field.name}>
...
//                         )}
//                         <label className={`ml-2 text-sm font-medium ${isFormDisabled ? 'text-gray-700' : 'text-gray-700'}`}>
//                           TOEFL Required: {formData.toefl_required ? 'Yes' : 'No'}
//                         </label>
//                       </div>
//                     </div>
...
//                       <div className="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-5 gap-3 mt-3">
//                         {[
//                           { label: "Reading", name: "toefl_reading", value: formData.toefl_reading },
//                           { label: "Writing", name: "toefl_writing", value: formData.toefl_writing },
//                           { label: "Listening", name: "toefl_listening", value: formData.toefl_listening },
//                           { label: "Speaking", name: "toefl_speaking", value: formData.toefl_speaking },
//                           { label: "Overall", name: "toefl_overall", value: formData.toefl_overall },
//                         ].map((field) => (
//                           <div key={field.name}>
//                             <label className="block text-xs font-medium text-gray-700 mb-1">
//                               {field.label}
...
//                       6
//                     </div>
//                     <h3 className="text-lg font-semibold text-secondary">Document Information</h3>
//                   </div>
                  
//                   <div className="space-y-4">
//                     {(['resume', 'passport_copy', 'transcripts', 'english_test', 'photo'] as const).map((field) => (
//                       <div key={field} className="border border-gray-200 rounded-lg p-4">
//                         <div className="flex justify-between items-center mb-2">
//                           <label className="block text-sm font-medium text-gray-700 capitalize">
//                             {field.replace('_', ' ')} {['resume', 'passport_copy', 'transcripts'].includes(field) && '*'}
//                           </label>
...
//                                 {filePreviews[field] && (
//                                   <div className="mt-2">
//                                     <img src={filePreviews[field]} alt={field} className="max-h-40 rounded-lg" />
//                                   </div>
//                                 )}
//                               </div>
//                             ) : files[field] ? (
...
//                             {formData[field as keyof FormData] && !files[field] && (
...
//                       7
//                     </div>
//                     <h3 className="text-lg font-semibold text-secondary">Test Scores</h3>
//                   </div>
...
//                       <h4 className="text-md font-medium text-gray-700">Additional Test Scores</h4>
//                       {!isFormDisabled && (
//                         <button
//                           type="button"
//                           onClick={() => addArrayItem('test_scores', { test_name: '', score: '', date: '' })}
...
//                               <h5 className="font-medium text-gray-700">Test Score #{index + 1}</h5>
//                               {!isFormDisabled && (
//                                 <button
//                                   type="button"
//                                   onClick={() => removeArrayItem('test_scores', index)}
...
//                                     onChange={(e) => handleArrayChange('test_scores', index, 'test_name', e.target.value)}
...
//                                     type="text"
//                                     value={test.score}
//                                     onChange={(e) => handleArrayChange('test_scores', index, 'score', e.target.value)}
//                                     className="w-full px-3 py-1.5 border border-gray-300 rounded-lg text-sm"
//                                     placeholder="Score"
//                                   />
//                                 )}
...
//                                     type="date"
//                                     value={test.date}
//                                     onChange={(e) => handleArrayChange('test_scores', index, 'date', e.target.value)}
//                                     className="w-full px-3 py-1.5 border border-gray-300 rounded-lg text-sm"
//                                   />
//                                 )}
//                               </div>
...
//                   <div className="space-y-4">
//                     <div className="flex justify-between items-center">
//                       <h4 className="text-md font-medium text-gray-700">Work Experiences</h4>
//                       {!isFormDisabled && (
//                         <button
//                           type="button"
//                           onClick={() => addArrayItem('work_experiences', {
//                             organization: '',
//                             position: '',
//                             start_date: '',
//                             end_date: '',
...
//                                     onChange={(e) => handleArrayChange('work_experiences', index, 'organization', e.target.value)}
//                                     className="w-full px-3 py-1.5 border border-gray-300 rounded-lg text-sm"
//                                   />
//                                 )}
//                               </div>
...
//                                     type="text"
//                                     value={exp.position}
//                                     onChange={(e) => handleArrayChange('work_experiences', index, 'position', e.target.value)}
//                                     className="w-full px-3 py-1.5 border border-gray-300 rounded-lg text-sm"
//                                   />
//                                 )}
//                               </div>
...
//                                     onChange={(e) => handleArrayChange('work_experiences', index, 'start_date', e.target.value)}
//                                     className="w-full px-3 py-1.5 border border-gray-300 rounded-lg text-sm"
//                                   />
//                                 )}
//                               </div>
...
//                                     type="date"
//                                     value={exp.end_date}
//                                     onChange={(e) => handleArrayChange('work_experiences', index, 'end_date', e.target.value)}
//                                     className="w-full px-3 py-1.5 border border-gray-300 rounded-lg text-sm"
//                                   />
//                                 )}
//                               </div>
...
//                                     onChange={(e) => handleArrayChange('work_experiences', index, 'description', e.target.value)}
//                                     rows={2}
//                                     className="w-full px-3 py-1.5 border border-gray-300 rounded-lg text-sm"
//                                   />
//                                 )}
...
//                       9
//                     </div>
//                     <h3 className="text-lg font-semibold text-secondary">References & SOP</h3>
//                   </div>
...
//                         <button
//                           type="button"
//                           onClick={() => addArrayItem('academic_qualifications', {
//                             degree: '',
//                             institution: '',
//                             year: '',
//                             cgpa: ''
...
//                   <div className="space-y-4">
//                     <div className="flex justify-between items-center">
//                       <h4 className="text-md font-medium text-gray-700">References</h4>
//                       {!isFormDisabled && (
//                         <button
//                           type="button"
...
//                       10
//                     </div>
//                     <h3 className="text-lg font-semibold text-secondary">Review & Submit</h3>
//                   </div>
                  
//                   <div className="bg-blue-50 border border-blue-100 rounded-lg p-4">
//                     <h4 className="text-base font-medium text-secondary mb-3">Review Your Application</h4>
//                     <p className="text-secondary text-sm mb-4">Please review all information before submitting.</p>
                    
//                     <div className="space-y-4">
//                       <div>
//                         <h5 className="font-medium text-gray-700 mb-2">Personal Information</h5>
//                         <div className="grid grid-cols-2 gap-2 text-sm">
//                           <div><span className="font-medium">Name:</span> {formData.student_name || 'N/A'}</div>
//                           <div><span className="font-medium">Student ID:</span> {formData.student_id || 'N/A'}</div>
...
//                           <div><span className="font-medium">Passport:</span> {formData.passport || 'N/A'}</div>
//                           <div><span className="font-medium">DOB:</span> {formData.dob || 'N/A'}</div>
//                         </div>
//                       </div>
                      
//                       <div>
//                         <h5 className="font-medium text-gray-700 mb-2">Academic Information</h5>
//                         <div className="grid grid-cols-2 gap-2 text-sm">
//                           <div><span className="font-medium">Study Level:</span> {formData.study_level || 'N/A'}</div>
//                           <div><span className="font-medium">Last Level:</span> {formData.last_level_of_study || 'N/A'}</div>
//                           <div><span className="font-medium">Qualifications:</span> {formData.academic_qualifications.length}</div>
//                           <div><span className="font-medium">Test Scores:</span> {formData.test_scores.length}</div>
//                         </div>
//                       </div>
                      
//                       <div>
//                         <h5 className="font-medium text-gray-700 mb-2">Program Details</h5>
//                         <div className="grid grid-cols-2 gap-2 text-sm">
//                           <div><span className="font-medium">University:</span> {formData.university_name || 'N/A'}</div>
//                           <div><span className="font-medium">Program:</span> {formData.program_name || 'N/A'}</div>
//                           <div><span className="font-medium">Intake:</span> {formData.intake || 'N/A'}</div>
//                           <div><span className="font-medium">Status:</span> {formData.status || 'N/A'}</div>
//                         </div>
//                       </div>
...
//                         <div className="grid grid-cols-2 gap-2 text-sm">
//                           <div><span className="font-medium">Resume:</span> {files.resume || formData.resume ? 'Uploaded' : 'Missing'}</div>
//                           <div><span className="font-medium">Passport Copy:</span> {files.passport_copy || formData.passport_copy ? 'Uploaded' : 'Missing'}</div>
//                           <div><span className="font-medium">Transcripts:</span> {files.transcripts || formData.transcripts ? 'Uploaded' : 'Missing'}</div>
//                           <div><span className="font-medium">Total References:</span> {formData.references.length}</div>
//                         </div>
//                       </div>
//                     </div>
...
//                           Application Date: {formData.created_at ? new Date(formData.created_at).toLocaleDateString() : new Date().toLocaleDateString()}
//                         </span>
//                       </div>
//                     </div>
//                   </div>
...
//                             After submission, you cannot modify this application. All changes must be requested through your admin.
//                           </p>
//                         </div>
//                       </div>
...
//                           />
//                         </div>
//                         <label htmlFor="confirmation-checkbox" className="ml-3 text-sm text-gray-700">
...
//                         </svg>
//                         <p className="text-gray-600 font-medium">This application has been submitted</p>
//                         <p className="text-gray-500 text-sm mt-1">Please contact your admin for any changes</p>
//                       </div>
//                     </div>
//                   )}
//                 </div>
...
const AgentFinalApply = ({
  ...
  // Save as Draft (Update only - keeps status as Review/Pending)
