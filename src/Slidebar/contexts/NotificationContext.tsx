...
// Helper function to map API notification to interface
const mapApiNotificationToInterface = (apiNotif: any): Notification => {
  return {
    id: apiNotif.id,
    title: apiNotif.subject || 'Notification',
    message: apiNotif.description || 'No message content',
    type: apiNotif.type || 'general',
    is_read: Boolean(apiNotif.is_read),
    created_at: apiNotif.created_at,
    read_at: apiNotif.read_at || null,
    metadata: {
      student_name: apiNotif.student_name,
      status: apiNotif.status,
      application: apiNotif.application,
      subject: apiNotif.subject,
...
export const NotificationProvider: React.FC<{ children: React.ReactNode }> = ({
  ...
  const fetchNotifications = async () => {
    ...
    try {
      ...
      if (result.success) {
        if (result.notifications && Array.isArray(result.notifications)) {
          // MAP THE API DATA TO MATCH YOUR INTERFACE
          notificationsData = result.notifications.map((apiNotif: any) => ({
            id: apiNotif.id,
            title: apiNotif.subject || 'Notification',
            message: apiNotif.description || 'No message content',
            type: apiNotif.type || 'general',
          ...
          notificationsData = result.data.map((apiNotif: any) => ({
            id: apiNotif.id,
            title: apiNotif.subject || 'Notification',
            message: apiNotif.description || 'No message content',
            type: apiNotif.type || 'general',
          ...
          notificationsData = result.map((apiNotif: any) => ({
            id: apiNotif.id,
            title: apiNotif.subject || 'Notification',
            message: apiNotif.description || 'No message content',
            type: apiNotif.type || 'general',
        ...
        if (notificationsData.length > 0) {
          console.log("First mapped notification:", notificationsData[0]);
        }
