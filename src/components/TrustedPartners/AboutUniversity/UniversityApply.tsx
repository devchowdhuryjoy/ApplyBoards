...
const UniversityApply: React.FC = () => {
  ...
  const [error, setError] = useState<string | null>(null);
  const navigate = useNavigate();
  const [openPopupId, setOpenPopupId] = useState<number | null>(null); // শুধু একটা কার্ডের জন্য popup
  ...
  // Create Application বাটন ক্লিক হ্যান্ডলার
  const handleCreateApplication = (universityId?: number) => {
    const isLoggedIn = checkLoginStatus();
    
    if (isLoggedIn) {
      // লগিন থাকলে সরাসরি ফর্ম পেজে নিয়ে যাও
      navigate("/create-application-form", {
        state: { universityId: universityId }
      });
    } else {
      // লগিন না থাকলে popup দেখাও (শুধু সেই কার্ডের জন্য)
      setOpenPopupId(universityId || null);
    }
  };

  // Popup থেকে লগিনে যাওয়ার হ্যান্ডলার
  const handleLogin = (userType: "student" | "agent") => {
    setOpenPopupId(null); // popup বন্ধ করুন
    if (userType === "student") {
      navigate("/login", {
        state: { redirectTo: "/create-application-form" },
      });
    } else {
  ...
  return (
    <>
            ...
            {currentUniversities.map((uni, i) => {
              ...
              return (
                ...
                >
                  ...
                  <div className="p-7 border-t border-gray-200">
                    <button
                      onClick={() => handleCreateApplication(uni.id)}
                      className="w-full bg-primary text-white font-semibold py-3.5 rounded-xl transition-all duration-300 shadow-md hover:bg-secondary"
                    >
                      Create Application
                    </button>
      ...
      {/* Global Popup Modal - শুধুমাত্র লগিন না থাকলে দেখাবে */}
      {openPopupId !== null && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 px-4">
          <div className="bg-white w-full max-w-md rounded-2xl p-6 relative animate-fadeIn">
            {/* Close */}
            ...
            <div className="mt-6 space-y-4">
              ...
              <button
                ...
                className="w-full border border-primary text-primary font-semibold py-3 rounded-xl hover:bg-[#252364] hover:text-white transition flex items-center justify-center gap-2"
              >
                <span>🎓</span>
                <span>Student</span>
              </button>
              ...
              <button
                ...
                className="w-full bg-primary text-white font-semibold py-3 rounded-xl hover:opacity-90 transition flex items-center justify-center gap-2"
              >
                <span>🧑‍💼</span>
                <span>Agent</span>
              </button>
